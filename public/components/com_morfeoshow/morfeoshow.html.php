...
function mostraFrontend($option, $rows, $pageNav, $limitstart, $limit, $total) {
	...
	if ($params->get('show_page_title', 1) && false) :
?>
...
<?php if ($params_morfeo['logo'] == 1 && false) {	?>
		<div align="left">
...
<ul style="list-style: none; margin: 0; padding:0;">
	<?php
		...
		foreach ($rows as $index=>$row) {
			$_clear = ($index%4 === 0)? 'clear:both;': '';
		echo "<li class='". $tabclass[$k] ."' style='margin:4px 4px;padding:0;width: 130px; float: left;".$_clear." asd".($index%4)."'>";
		if ($row->shortcut_filename) {
		?>
			<div style="text-align: center;">
				<a href="index.php?option=<?php echo $option; ?>&task=view&gallery=<?php echo $row->id; ?>">
				<img border="0" src="images/morfeoshow/<?php echo $row->folder; ?>/<?php echo $row->shortcut_filename; ?>"/></a>
			</div>
	<?php } else {	?>
			<div style="text-align: center;">
				<a href="index.php?option=<?php echo $option; ?>&task=view&gallery=<?php echo $row->id; ?>">
				<img border="0" src="<?php JURI::root()?>components/com_morfeoshow/images/no.png"/></a>
			</div>
	<?php }	?>
			<div style="text-align: center;">
				<a href="index.php?option=<?php echo $option; ?>&task=view&gallery=<?php echo $row->id; ?>">
